...
<input type="hidden" id="sortKeyword" name="sortKeyword" class="input_text" value="${param.sortKeyword}" readonly  />
	<!-- 상단 이미지 -->
                ...
                <form action="${path}/room/list" autocomplete="off">
                    <div class="row" style="justify-content: center;">
                        ...
                        <div class="col-xl-3 col-md-6 mb-4">
                          ...
                          <option value="강서구" <c:if test="${param.place == '강서구'}"> selected="selected" </c:if>>강서구</option>
                          <option value="북구" <c:if test="${param.place == '북구'}"> selected="selected" </c:if>>북구</option>
                          <option value="금정구" <c:if test="${param.place == '금정구'}"> selected="selected" </c:if>>금정구</option>
                          <option value="기장군" <c:if test="${param.place == '기장군'}"> selected="selected" </c:if>>기장군</option>
                          <option value="사상구" <c:if test="${param.place == '사상구'}"> selected="selected" </c:if>>사상구</option>
                          <option value="부산진구" <c:if test="${param.place == '부산진구'}"> selected="selected" </c:if>>부산진구</option>
                          <option value="연제구" <c:if test="${param.place == '연제구'}"> selected="selected" </c:if>>연제구</option>
                          <option value="동래구" <c:if test="${param.place == '동래구'}"> selected="selected" </c:if>>동래구</option>
                          <option value="사하구" <c:if test="${param.place == '사하구'}"> selected="selected" </c:if>>사하구</option>
                          <option value="서구" <c:if test="${param.place == '서구'}"> selected="selected" </c:if>>서구</option>
                          <option value="동구" <c:if test="${param.place == '동구'}"> selected="selected" </c:if>>동구</option>
                          <option value="남구" <c:if test="${param.place == '남구'}"> selected="selected" </c:if>>남구</option>
                          <option value="중구" <c:if test="${param.place == '중구'}"> selected="selected" </c:if>>중구</option>
                          <option value="수영구" <c:if test="${param.place == '수영구'}"> selected="selected" </c:if>>수영구</option>
                          <option value="해운대구" <c:if test="${param.place == '해운대구'}"> selected="selected" </c:if>>해운대구</option>
                          <option value="영도구" <c:if test="${param.place == '영도구'}"> selected="selected" </c:if>>영도구</option>
                        </select>
                        </div>
                        ...
                        <div class="col-xl-3 col-md-6 mb-4">
                            ...
                            <div class="datepicker-container datepicker-container-left">
                                <input class="form-control" type="text" name="bookingDate" id="bookingDate" placeholder="날짜 선택" value="${param.bookingDate}">
                            </div>
                        </div>
                ...
                <div class="d-flex justify-content-between align-items-center flex-column flex-md-row mb-4">
                    ...
                    <div>
                       ...
                       <select class="selectpicker " name="sortKeyword" id="sortKeyword" data-style="btn-selectpicker " title="정렬" onchange="movePage2(this.value)">                     	
                          		 <c:if test="${empty param.sortKeyword}">
                          		 <option value="DESC" selected >조회수 ▲   </option>
                				<option value="ASC">조회수 ▼   </option>
                				</c:if>
                				
                          		 <c:if test="${param.sortKeyword eq 'DESC'}">
                          		 <option value="DESC" selected >조회수 ▲   </option>
                				<option value="ASC">조회수 ▼   </option>
                				</c:if>
                				
                				<c:if test="${param.sortKeyword eq 'ASC'}">
                          		 <option value="DESC" >조회수 ▲   </option>
                				<option value="ASC" selected>조회수 ▼   </option>
                				</c:if>
                        </select>
                    </div>
                </div>
...
function movePage(pageUrl){
	...
	var sort = document.getElementById("sortKeyword");
	...
	if(sort != null && sort.value.length > 0){
		pageUrl = pageUrl + '&sortKeyword=' + sort.value;
	}
